...
Every page carries one consistent visual identity, implemented in
`assets/css/signature.css` + `assets/js/pendulum.js` +
`assets/js/scroll-reveal.js`, and linked/loaded on every page (`index`,
`about`, `projects`, `contact`, `404`):

- **Blueprint backdrop** (`.blueprint-bg`) — a fixed, near-invisible CAD-style
  construction grid (`--blueprint-opacity: 0.05`). Texture, not pattern.
- **Dust** (`.dust`, `.dust__mote`) — a handful of tiny motes drifting
  upward on independent randomized durations/delays, set inline per-mote so
  no two pages feel mechanical.
- **Pendulum** (`.pendulum` + canvas, driven by `pendulum.js`) — a real
  small-angle pendulum simulation. Scroll deltas add angular-velocity
  "kicks"; damping increases over the last ~12% of page height so it settles
  to rest by the time the reader reaches the bottom rather than being cut off
  mid-swing. Colors are read from the `--pendulum-*` tokens via
  `getComputedStyle`, never hardcoded in JS.
- **Scroll reveal** (`[data-reveal]`, driven by `scroll-reveal.js`) —
  below-the-fold section headings fade/rise in once in view, flushed on the
  pendulum's `pendulum:cross` event (dispatched each time the bob crosses
  center) so the two motions read as one system. Falls back to a 900ms
  safety-net timer if the event never arrives.

All three decorative layers are `aria-hidden="true"`, `pointer-events: none`,
and fully removed or neutralized under `prefers-reduced-motion: reduce`
(pendulum freezes at rest with no scroll listener attached; dust/blueprint
are hidden outright; `[data-reveal]` content is shown immediately, no
transition).

**Stacking note:** the three layers are `position: fixed` with explicit
`z-index: 0/1/2`. Per the CSS stacking spec, any positioned element at
`z-index: 0+` paints above in-flow, non-positioned content regardless of DOM
order — so `#main` and `.site-footer` are given `position: relative;
z-index: var(--signature-z)` in `signature.css` to sit above them.
`.site-header` doesn't need this: it already sets `position: sticky;
z-index: 50` in `components.css`, which clears the pendulum on its own.
